...
`git clone https://github.com/ByteLegendQuest/java-set-a-conditional-breakpoint`

A project directory named `java-set-a-conditional-breakpoint` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-set-a-conditional-breakpoint` directory which is created just now.
